...
export async function updateCartIcon() {
    try {
        ...
        document.getElementById('cart-banner').innerHTML = totalItems > 0 ? `<a href='/cart.html'><img src='images/cart.png' alt='cart'/>${totalItems}</a>` 
        : ''
    } catch (error) {
        console.log('error when updating the icon',error)
    }
}
...
export async function loadCart(dom) {
    ...
    try {
        ...
        cartList.innerHTML = '<li>Error loading the card data</li>'
    }
}
...
async function fetchCartItems({ userMessage, checkoutBtn}) {
    ...
    if(!res.ok) {
        ...
        userMessage.innerHTML = 'Please <a href="/login">log in</a>'
        return []
    }
...
export async function removeItem(itemId, dom) {
    try {
        ...
        if(res.status === 204) {
            ...
            console.error('Error handling removing item', await res.text())
        }
    } catch (error) {
